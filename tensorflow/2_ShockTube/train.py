...
qry_res      = 2048
ltt_res      = 1024
en_loc       = 4 # locality paramerter in Encoder
de_loc       = 2 # locality paramerter in Encoder
save_path = './model/'
# load dataset
trainX, trainY, testX, testY= load_data("./2_ShockTube.mat", n_train, n_test)
...
network   = PiT(m_query, m_cross, m_latent, out_dim, encode_dim, n_head, en_loc, de_loc)
#network   = LiteTransformer(m_query, m_cross, out_dim, encode_dim, n_head, en_loc, de_loc)
...
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=tf.keras.optimizers.schedules.CosineDecay(lr, n_epochs*(n_train//batch_size))), 
                                                 loss=rel_norm(),
                                                 jit_compile=True) # jit_compile always on, for faster training
